...
https://www.kaggle.com/etsc9287/a-thorough-eda-on-ashrae
ggplot2 styles:
...
To imputate the data for the air_temperature 2 methods will performed:
- When a missing value is found pick the two colindant values (previous and later) and make the mean.
- If the two values are missing, pick the value of the same hour but from the previous day an next day and mean

```{r}
imputation_table <- weather_train %>% 
  group_by (site_id) %>%
  mutate(previous_temp = lag(air_temperature, n=1), next_temp = lead(air_temperature, n=1),
         last_day_same_hour_temp = lag(air_temperature, n=24), next_day_same_hour_temp = lead(air_temperature, n=24)) %>% select(site_id, air_temperature,previous_temp, next_temp, last_day_same_hour_temp, next_day_same_hour_temp)

```

```{r}
imputation_table_mean <- imputation_table


condition = is.na(imputation_table_mean$air_temperature)
imputation_table_mean[condition,"air_temperature"] = rowMeans(imputation_table_mean[condition,c("previous_temp","next_temp")], na.rm= TRUE)

condition = is.na(imputation_table_mean$air_temperature)
imputation_table_mean[condition,"air_temperature"] = rowMeans(imputation_table_mean[condition,c("last_day_same_hour_temp","next_day_same_hour_temp")], na.rm= TRUE)
condition = is.na(imputation_table_mean$air_temperature)
sum(condition)
...
- Mice imputation:
Amb les mateixes variables creades fem apliquem el MICE. Aquí explicaré el que fa el MICE
